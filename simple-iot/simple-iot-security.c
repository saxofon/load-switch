...
int simple_security_init(const char *privkey, const int privkey_len, const char *pubkey, const int pubkey_len)
{
	int status;
...
int simple_security_cleanup(void)
{
...
int simple_create_signature(mbedtls_pk_context* pk, char* buf, int buf_len, char** sig, size_t* sig_len)
{
	int status;
...
int simple_verify_signature(mbedtls_pk_context *pk, char *buf, int buf_len, char *sig, int sig_len)
{
	int status;
	unsigned char hash[32];
